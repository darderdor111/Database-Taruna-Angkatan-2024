<!-- Admin: Form Edit Data Taruna -->
<!DOCTYPE html><html class="light" lang="en">
<head>
<meta charset="utf-8">
<meta content="width=device-width, initial-scale=1.0" name="viewport">
<title>STMKG 2024 - Edit Data Taruna</title>
...
<!-- Header with Accent -->
<div class="h-1.5 bg-[#d97706] w-full"></div>
<div class="p-6 border-b border-outline-variant bg-surface-container-low/30">
<div class="flex items-center gap-4">
<div class="w-12 h-12 bg-[#fff7ed] rounded flex items-center justify-center border border-[#d97706]/20">
<span class="material-symbols-outlined text-[#d97706] text-[32px]">edit_document</span>
</div>
<div>
<h2 class="font-headline-md text-title-lg text-[#d97706] uppercase tracking-tight">Edit Data Taruna</h2>
<p class="font-body-md text-body-md text-on-surface-variant">Perbarui informasi taruna di bawah ini.</p>
</div>
</div>
</div>
<!-- Registration Form -->
<form class="p-8 space-y-6" action="{{ route('admin.taruna.update', $taruna->id_taruna) }}" method="POST">
    @csrf
    @method('PUT')
...
<input name="npt" value="{{ old('npt', $taruna->npt) }}" class="block w-full h-11 px-4 bg-white border border-outline-variant rounded focus:ring-2 focus:ring-primary focus:border-transparent text-body-md transition-all placeholder:text-outline-variant" id="npt" placeholder="Contoh: 21.24.0001" required="" type="text">
</div>
<!-- Nama Lengkap -->
<div class="space-y-1.5">
...
<input class="form-radio w-4 h-4 text-primary border-outline focus:ring-primary" name="gender" required="" type="radio" value="L" {{ old('gender', $taruna->jenis_kelamin) == 'L' ? 'checked' : '' }}>
<span class="ml-2 font-body-md text-on-surface group-hover:text-primary transition-colors">Laki-laki</span>
</label>
<label class="inline-flex items-center group cursor-pointer">
<input class="form-radio w-4 h-4 text-primary border-outline focus:ring-primary" name="gender" required="" type="radio" value="P" {{ old('gender', $taruna->jenis_kelamin) == 'P' ? 'checked' : '' }}>
...
<script>
        ...
        // Setup state awal dari database
        const initialKelas = "{{ old('id_kelas', $taruna->id_kelas) }}";
        if(initialKelas) {
            const initialOption = originalKelasOptions.find(opt => opt.value == initialKelas);
            if(initialOption) {
                prodiSelect.value = initialOption.getAttribute('data-prodi');
                kelasSelect.value = initialKelas;
            }
        }

        function updateKelasOptions(resetValue = false) {
            const selectedProdi = prodiSelect.value;
            const currentKelas = resetValue ? '' : kelasSelect.value;
            ...
            if(!currentKelas) placeholder.selected = true;
            ...
            originalKelasOptions.forEach(option => {
                if (option.value && (!selectedProdi || option.getAttribute('data-prodi') === selectedProdi)) {
                    ...
                    if (cloned.value == currentKelas) {
                        cloned.selected = true;
                        found = true;
                    }
                    kelasSelect.appendChild(cloned);
                }
            });
            
            if(!found && currentKelas !== '') {
                kelasSelect.value = '';
            }
        }

        prodiSelect.addEventListener('change', () => {
            updateKelasOptions(true);
        });
        
        // Run once on load
        updateKelasOptions(false);
    </script>
</body></html>
